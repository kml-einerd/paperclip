...
function resolveAuthToken(config: Record<string, unknown>, headers: Record<string, string>): string | null {
  ...
  const tokenHeader = headerMapGetIgnoreCase(headers, "x-pmos-token");
  if (nonEmpty(tokenHeader)) return nonEmpty(tokenHeader);

  const authHeader =
    headerMapGetIgnoreCase(headers, "x-pmos-auth") ??
...
export async function testEnvironment(
  ...
  if (!urlValue) {
    checks.push({
      ...
      level: "error",
      message: "Pmos gateway adapter requires a WebSocket URL.",
  ...
  if (url && url.protocol !== "ws:" && url.protocol !== "wss:") {
    checks.push({
      code: "pmos_gateway_url_protocol_invalid",
      level: "error",
  ...
  if (url) {
    ...
    if (url.protocol === "ws:" && !isLoopbackHost(url.hostname)) {
      checks.push({
        code: "pmos_gateway_plaintext_remote_ws",
        level: "warn",
  ...
  if (authToken || password) {
    checks.push({
      code: "pmos_gateway_auth_present",
      level: "info",
      message: "Gateway credentials are configured.",
    });
  } else {
    checks.push({
      code: "pmos_gateway_auth_missing",
      level: "warn",
      message: "No gateway credentials detected in adapter config.",
      hint: "Set authToken/password or headers.x-pmos-token for authenticated gateways.",
    });
  }
  ...
  if (url && (url.protocol === "ws:" || url.protocol === "wss:")) {
    try {
      ...
      if (probeResult === "ok") {
        ...
        checks.push({
          code: "pmos_gateway_probe_failed",
          level: "warn",
