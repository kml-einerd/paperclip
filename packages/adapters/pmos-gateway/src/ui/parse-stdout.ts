...
import { normalizePmosGatewayStreamLine } from "../shared/stream.js";
...
function parseAgentEventLine(line: string, ts: string): TranscriptEntry[] {
  const match = line.match(/^\[pmos-gateway:event\]\s+run=([^\s]+)\s+stream=([^\s]+)\s+data=(.*)$/s);
...
export function parsePmosGatewayStdoutLine(line: string, ts: string): TranscriptEntry[] {
  const normalized = normalizePmosGatewayStreamLine(line);
  ...
  if (trimmed.startsWith("[pmos-gateway:event]")) {
    return parseAgentEventLine(trimmed, ts);
  }

  if (trimmed.startsWith("[pmos-gateway]")) {
    return [{ kind: "system", ts, text: trimmed.replace(/^\[pmos-gateway\]\s*/, "") }];
  }
