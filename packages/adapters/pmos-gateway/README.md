...
This document describes how `@paperclipai/adapter-pmos-gateway` invokes OpenClaw over the Gateway protocol.
...
- `[pmos-gateway] ...` for lifecycle/system logs
- `[pmos-gateway:event] run=<id> stream=<stream> data=<json>` for `event agent` frames
