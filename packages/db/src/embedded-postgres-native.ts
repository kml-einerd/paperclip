...
/**
 * Resolve the absolute path to a bundled embedded-postgres binary (e.g.
 * `pg_ctl`, `postgres`, `initdb`). Returns null when the platform is
 * unsupported or the native package cannot be located. Used for a clean
 * `pg_ctl stop -m fast` shutdown that fully releases shared memory.
 */
export function resolveEmbeddedPostgresBinPath(binary: "pg_ctl" | "postgres" | "initdb"): string | null {
  const nativePackageName = resolveNativePackageName();
  const packageRoot = resolveEmbeddedPostgresPackageRoot();
  if (!nativePackageName || !packageRoot) return null;

  const nativeRoot = path.resolve(packageRoot, "..", "@embedded-postgres", nativePackageName);
  return path.join(nativeRoot, "native", "bin", binary);
}
